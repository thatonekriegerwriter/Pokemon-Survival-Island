...
class HallOfFame_Scene # Minimal change to store HoF time into a variable
  ...
  def writeTrainerData
    ...
    hour     = totalsec / 60 / 60
    min      = totalsec / 60 % 60
    # Store time of first Hall of Fame in $Trainer.hall_of_fame if not array is empty
    if $Trainer.hall_of_fame = []
      $Trainer.hall_of_fame.push(pbGetTimeNow)
      $Trainer.hall_of_fame.push(totalsec)
    end
...
class PokemonTrainerCard_Scene2
  ...
  def pbDrawTrainerCardFront2
    ...
    textPositions = [
       ...
       [$Trainer.name,302+89*2,58-16,1,baseColor,shadowColor],
       [_INTL("ID No."),32,58-16,0,baseColor,shadowColor],
       [sprintf("%05d",$Trainer.public_ID),468-122*2,64-16,1,baseColor,shadowColor],
       ...
       [sprintf("%d",$Trainer.score),302+2,202,1,baseColor,shadowColor],
  ...
  def pbDrawTrainerCardBack2
    ...
    if $Trainer.hall_of_fame != []
      hof.push(_INTL("{1} {2}, {3}",
        pbGetAbbrevMonthName($Trainer.hall_of_fame[0].mon),
        $Trainer.hall_of_fame[0].day,
        $Trainer.hall_of_fame[0].year))
      hour = $Trainer.hall_of_fame[1] / 60 / 60
      min  = $Trainer.hall_of_fame[1] / 60 % 60
      time = _ISPRINTF("{1:02d}:{2:02d}",hour,min)
      hof.push(time)
    else
      hof.push("--- --, ----")
    ...
    textPositions = [
      ...
      [_INTL($Trainer.fullname2),32,106-16,0,baseColor,shadowColor],
    ...
    x = 64-28
    for i in 0...8
      imagepos.push(["Graphics/Pictures/Trainer Card/badges0",x,104*2,i*48,0*48,48,48]) if $Trainer.badges[i+0*8]
      x += 48+8
    end
    # Draw Region 1 badges
    x = 64-28
    for i in 0...8
      imagepos.push(["Graphics/Pictures/Trainer Card/badges1",x,104*2+52,i*48,0*48,48,48]) if $Trainer.badges[i+1*8]
      x += 48+8
    end
