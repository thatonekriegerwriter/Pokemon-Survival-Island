...
def pbBedCore
    pbMessage(_INTL("You sleep for {1} hour(s).",$game_variables[247]))
	...
	pbRandomEvent
    if pbPokerus? == true
    pbMessage(_INTL("Your Pokemon may be infected by PokeRus."))
    pbMessage(_INTL("Little is known about the PokeRus except that they are microscopic life-forms that attach to Pokémon.."))
    pbMessage(_INTL("While infected, Pokémon are said to grow exceptionally well."))
    end
	if $game_switches[157]==true && $game_variables[423]>=1
	  if pbWinLoseChecker == true
       $Trainer.money += 500
	  end
	end
    $game_variables[29]=$game_variables[247]*3840
	pbSleepRestore
