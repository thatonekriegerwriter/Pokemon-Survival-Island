...
def make_party_validator(pbs_dir):
    ...
    with io.open(os.path.join(pbs_dir, r'pokemon.txt'), 'r', encoding='utf-8-sig') as pokemon_pbs:
        ...
        for number in pokemon_pbs_.sections():
            ...
            if species['InternalName'] in form_tutor_by_name:
                moves |= {m for m in form_tutor_by_name[species['InternalName']].split(',') if m}
            if species['InternalName'] in form_egg_by_name:
                moves |= {m for m in form_egg_by_name[species['InternalName']].split(',') if m}
            pokemon_by_name[species['InternalName']] = Pokemon(genders, abilities, moves, forms_by_name.get(species['InternalName'], default_forms))
