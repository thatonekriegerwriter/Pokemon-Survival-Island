...
class DiscordAPI
  def initialize
    # API references
    @func_discord_initialize = MiniFFI.new(DLL, "Discord_Init", 'l', 'v')
  ...
  def run_callbacks
    @func_discord_update.call
  end
  
  def update
    @func_discord_activities_update.call(@state,@details,@large_image,@small_image,@large_image_text,@small_image_text, @timestamp_start, @timestamp_end, @party_size, @party_max)
  end
