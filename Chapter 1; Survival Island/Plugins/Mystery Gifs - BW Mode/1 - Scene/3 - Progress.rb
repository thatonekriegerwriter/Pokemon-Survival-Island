		...
		def receive_gift_anim
			...
			file =
      		...
      		species = GameData::Species.get(species).species
					GameData::Species.front_sprite_filename(species, pkmn.form, pkmn.gender, pkmn.shiny?, pkmn.shadowPokemon?)
				else # Item
		...
		def show_wonder_card(got, date)
			...
			file =
				if got[1] == 0 # Pokemon
					...
					species = pkmn.species
      		species = GameData::Species.get(species).species
					GameData::Species.icon_filename(species, pkmn.form, pkmn.gender, pkmn.shiny?, pkmn.shadowPokemon?)
				else
					GameData::Item.icon_filename(got[2])
				end
