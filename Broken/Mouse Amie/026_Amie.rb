...
=begin
    def pbBackdrop #gets background based off location
      ...
      if environ==(:Cave)
        backdrop="Cave"
      elsif environ==(:MovingWater) || environ==(:StillWater)
        backdrop="Water"
        ...
        if !$game_map || !GameData::Metadata.get($game_map.map_id).outdoor_map
          backdrop="IndoorA"
        end
      end
      if $game_map
        back=GameData::Metadata.get.($game_map.map_id,battle_background)
        if back && back!=""
          backdrop=back
        end
      end
